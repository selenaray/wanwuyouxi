...
export function ClueSheet({ clue, onClose }: { clue: GameClue | GameEvidence; onClose: () => void }) {
  const evidence = "publicDescription" in clue;
  return (
    <div className="sheet-backdrop" role="presentation">
      <section className="clue-sheet" role="dialog" aria-modal="true" aria-labelledby="clue-title">
        ...
        <p>{evidence ? clue.publicDescription : clue.clueText}</p>
        <div className="evidence-tag"><span>现场证物</span><span>已录入档案</span></div>
        <button className="secondary-button" type="button" onClick={onClose}>{evidence ? "收起物证" : "收起线索"}</button>
      </section>
    </div>
  );
}
